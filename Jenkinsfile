...
pipeline {
    ...
    stages {
        ...
        stage('Merge values files') {
            steps{
                 script {
                     appconfig_values = sh (script: "ls ${WORKSPACE}/chart/eric-enmsg-access-control/appconfig/ | grep values.yaml", returnStatus: true)
                     ...
                     sh '''
                         if git status | grep 'values.yaml' > /dev/null; then
                            git add chart/eric-enmsg-access-control/values.yaml
    ...
    post {
        ...
        failure {
            mail to: 'PDLTEAMTOT@pdl.internal.ericsson.com',
